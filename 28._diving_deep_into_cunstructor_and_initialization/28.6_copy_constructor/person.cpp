...
// Delegate from the copy constructor. above constructor doing what we trying to do with copy constructor so handed(delegated) this work to above constructor. so we don't have to repeat and it's avoids duplication.
Person::Person(const Person &source_p)
