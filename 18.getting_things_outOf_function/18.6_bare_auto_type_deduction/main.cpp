...
2. double & some_var_ref {some_var} <- it is reference double. using it we can change the value of the some_var. because it is reference we have access to original variable.

3. auto y = some_var_ref <- now what do you think. what has auto done.
auto has only copied the value of the variable and provided type according to some_var_ref already. has it did't deduce the or copy the reference of double we have given to double some_var_ref. so using y we cannot change the original variable. it is not reference rather just value and type has been copied.

if you want reference from the auto to have access of the original variable. then you can do one thing.

auto& y = some_var_ref <- make auto reference.
