...
double sum(const double scores[][3], size_t);
[]<- we don't have to specify first size because it would be useless as we learned.

[3] <- we have to pass it.

if you are passing multidimensional then you have to specify for the size according to the dimension

as it is raw array we have to pass size as second argument.
*/
